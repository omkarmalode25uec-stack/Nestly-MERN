 ...
 * Owner verification & approval status dashboard
 */
 ...
 * Owner Payout & Escrow Settlement Hub
 */
...
/**
 * POST /owner/settlement
 * Update Owner Payout / Settlement Destination (UPI ID or Bank Account)
 * Security rule: Never request or accept PINs, passwords, OTPs, or merchant keys.
 */
...
/**
 * GET /owner/dashboard
 * Main Owner / Partner Operations Dashboard
 */
router.get('/dashboard', isLoggedIn, isOwner, async (req, res, next) => {
  try {
    // Only query properties belonging to the logged-in owner
    const properties = await Property.find({ owner: req.user._id }).sort({ createdAt: -1 });

    // Query all accommodation booking requests for this owner's properties
    const bookings = await Booking.find({ owner: req.user._id })
      .populate('property')
      .populate('user', 'name email phone college avatar')
      .sort({ createdAt: -1 });

    // Query Escrow Settlement records for this owner
    ...
    // Compute key operational statistics for the Stitch dashboard KPI cards
...
/**
 * POST /owner/properties/:id/toggle-status
 * Toggle availability / active status for an owner's property
 */
 ...
 * Delete an owner's property and return to dashboard
 */
 ...
 * Owner accepts student booking request
 */
router.post('/bookings/:id/accept', isLoggedIn, async (req, res, next) => {
  try {
    ...
    if (booking.status !== 'confirmed') {
      ...
      // Decrement available bed count if available
 ...
 * Owner rejects student booking request
 */
router.post('/bookings/:id/reject', isLoggedIn, async (req, res, next) => {
  try {
    ...
    // Security: Only the property owner can reject (or admin)
    if (!booking.owner.equals(req.user._id) && req.user.role !== 'admin') {
      req.flash('error', 'You are not authorized to manage this booking.');
      return res.redirect('/owner/dashboard');
    }

    // State check: cannot reject already cancelled requests
