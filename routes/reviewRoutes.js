 ...
 * Submit a verified review for a property
 */
router.post('/stays/:id/reviews', isLoggedIn, async (req, res, next) => {
  try {
    ...
    // 1. Conflict of Interest: Owner cannot review their own accommodation
    ...
    // 2. Duplicate Review Prevention: One review per user per property
    ...
    // 4. Update Property Average Rating & Review Count
...
/**
 * POST /stays/:id/reviews/:reviewId/delete
 * Delete review (Only authorized for the review author)
 */
router.post('/stays/:id/reviews/:reviewId/delete', isLoggedIn, async (req, res, next) => {
  try {
    ...
    // Authorization: ONLY the author of the review can delete it (or platform admin)
    ...
    if (remainingReviews.length > 0) {
      ...
      property.rating = 4.8; // default benchmark rating
